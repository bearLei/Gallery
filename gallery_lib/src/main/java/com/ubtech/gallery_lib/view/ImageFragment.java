 ...
 * desc:
 */
public class ImageFragment extends GalleryBaseFragment {
    ...
    private void subscribeUI() {
        LiveData<MediaResponse> imageLiveData = viewModel.getImageLiveData();
        if (null != imageLiveData) {
            imageLiveData.observe(getViewLifecycleOwner(), it -> {
                if (Constant.CODE_SUCCESS == it.getCode()) {
                    Log.d("leix", "获取图片成功：" + it.getData().size());
                    mData.clear();
                    mData.addAll(it.getData());
                    adapter.notifyDataSetChanged();
                } else {
                    Log.d("leix", "获取图片失败：" + it.getCode());
                }
            });
        }
    }
    ...
    private void requestData(){
        viewModel.getImageList(getActivity(), String.valueOf(Integer.MIN_VALUE),0,100);
    }
}
